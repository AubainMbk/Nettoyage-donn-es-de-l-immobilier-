...
--We have observed that some properties don't have an address but when we order data by ParcelID we see that the same parcelID have also the same property adress
--so our null property address have to be completed by the propertyID of the same parcelID, that's what we are trying to do.
...
-- We can do the same thing with owner address but we will do another method to train 
-- Let's use parsename
